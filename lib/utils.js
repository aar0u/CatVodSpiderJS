...
export function getCurrentFileName() {
    try {
        ...
        const matches = Array.from(stack.matchAll(/at\s+(?:\w+\s+)?\(?(.+?\.js)/g));
        const fileMatch = matches.length > 1 ? matches[1][1] : matches[0][1];
        if (!fileMatch) throw new Error("Could not parse filename from stack");
        return fileMatch.split("/").pop().split(".")[0];
    } catch (e) {
        console.log(`Error getting filename: ${e}`);
        return '_unknown';
    }
}
